...
export default function CalendarScreen({ navigation, route }) {
  ...
  // Add BackHandler for Android back button
useEffect(() => {
  const sub = BackHandler.addEventListener('hardwareBackPress', () => {
    if (presetPickerVisible) { setPresetPickerVisible(false); return true; }
    if (presetModalVisible)  { setPresetModalVisible(false);  return true; }
    if (logModalVisible) {
      if (logStep === 2) { setLogStep(1); return true; }
      setLogModalVisible(false);
      setEditingWorkout(null);
      setLogStep(1);
      return true;
    }
    if (shareWorkout) { setShareWorkout(null); return true; }
    return false; // let the tab navigator handle back normally
  });
  return () => sub.remove();
}, [logModalVisible, logStep, presetModalVisible, presetPickerVisible, shareWorkout]);
