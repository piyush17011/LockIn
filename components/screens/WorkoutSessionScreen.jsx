...
import { calcTotalCalories, formatDuration } from '../../constants/calorieCalc';
...
export default function WorkoutSessionScreen({ route, navigation }) {
  ...
  // ── Local exercises (isolated copy — removals never touch the preset) ────────
  const [exercises, setExercises] = useState(() => workout.exercises.map((ex) => ({ ...ex })));

  const removeExercise = (exIdx) => {
    Alert.alert(
      'Remove Exercise?',
      `Remove "${exercises[exIdx].name}" from this session? Your preset won't be affected.`,
      [
  ...
  const addSet = (exIdx) => {
    setSets((prev) => prev.map((exSets, i) =>
      i !== exIdx ? exSets : [...exSets, { weight: exSets[exSets.length-1]?.weight || '', reps: exSets[exSets.length-1]?.reps || '', done: false }]
    ));
  };
  ...
  const removeSet = (exIdx, setIdx) => {
    ...
    setSets((prev) => prev.map((exSets, i) =>
      i !== exIdx ? exSets : exSets.filter((_, j) => j !== setIdx)
    ));
  };
  ...
  // Rebuild exercise structure with current set data for calorie calc
  const liveExercises = exercises.map((ex, exIdx) => ({
    name: ex.name,
    sets: (sets[exIdx] || []).filter(s => s.done).map(s => ({ reps: s.reps, weight: s.weight })),
  }));
  ...
  const saveAndExit = async () => {
    ...
    // Only log exercises where at least 1 set was marked done
    const flat = exercises
      .map((ex, exIdx) => {
        const doneSetsForEx = (sets[exIdx] || []).filter((s) => s.done);
        if (doneSetsForEx.length === 0) return null;
        return {
          name: ex.name, emoji: ex.emoji, muscle: ex.muscle,
          sets: doneSetsForEx.length,
          reps: doneSetsForEx.map((s) => s.reps || '0').join('/'),
          weight: doneSetsForEx.map((s) => s.weight || '0').join('/'),
        };
      })
      .filter(Boolean);
    ...
    if (onSave) {
      onSave({ id: `temp_${Date.now()}`, userId: user.uid, ...workoutPayload });
    }

    navigation.goBack();
    logWorkout(user.uid, workoutPayload).catch((e) => console.error('Save failed', e));
  };
  ...
  const handleFinish = () => {
    ...
    if (isPartial) {
      Alert.alert(
        'Log Partial Workout? 📋',
        `Only ${doneSets}/${totalSets} sets completed.\nOnly what you've finished will be saved.\n\n${formatTime(elapsed)}  ·  ~${finalCalories} cal`,
        [
          { text: 'Keep Going', style: 'cancel' },
          { text: 'Log Partial', onPress: saveAndExit },
        ]
      );
    } else {
      Alert.alert(
        'Finish Workout? 🔥',
        `${doneSets}/${totalSets} sets  ·  ${formatTime(elapsed)}  ·  ~${finalCalories} cal`,
        [
          { text: 'Keep Going', style: 'cancel' },
    ...
    <View style={s.root}>
      ...
      <View style={s.restConfigRow}>
        ...
        {REST_OPTIONS.map((o) => (
          <TouchableOpacity
            key={o.value}
            style={[s.restChip, restDuration === o.value && s.restChipActive]}
        ...
        <View style={s.restBanner}>
          ...
          <View style={{ flex: 1 }}>
            ...
            <View style={s.restBarBg}>
              <RNAnimated.View style={[s.restBarFill, { width: restBarWidth }]} />
            </View>
          </View>
            ...
            return (
              <View key={exIdx} style={[s.exCard, allDone && s.exCardDone, isResting && s.exCardResting]}>
                  ...
                  return (
                    <View key={setIdx} style={[s.setRow, set.done && s.setRowDone, isThisResting && s.setRowResting]}>
                      <View style={[s.setNum, set.done && s.setNumDone]}>
                        {set.done
                          ? <Ionicons name="checkmark" size={13} color="#080b10" />
                ...
                <View style={s.setActions}>
                  ...
                  {sets[exIdx].length > 1 && (
                    <TouchableOpacity style={s.removeSetBtn} onPress={() => removeSet(exIdx, sets[exIdx].length - 1)}>
